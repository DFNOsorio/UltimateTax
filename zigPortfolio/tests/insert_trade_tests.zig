...
test "insert_trade_struct: valid insert round-trips values" {
    ...
    var t: api.zp_trade = std.mem.zeroes(api.zp_trade);

    _ = std.fmt.bufPrintZ(&t.trade_datetime, "{s}", .{"2025-12-27 13:00"}) catch unreachable;
    _ = std.fmt.bufPrintZ(&t.ticker, "{s}", .{"AAPL"}) catch unreachable;

    t.quantity = 3.0;
    t.price_per_share = 199.99;

    _ = std.fmt.bufPrintZ(&t.broker, "{s}", .{"IKBR"}) catch unreachable;
    _ = std.fmt.bufPrintZ(&t.trade_type, "{s}", .{"BUY"}) catch unreachable;
    t.commission = 0.5;
    _ = std.fmt.bufPrintZ(&t.country, "{s}", .{"US"}) catch unreachable;
    _ = std.fmt.bufPrintZ(&t.currency, "{s}", .{"USD"}) catch unreachable;
...
test "insert_trade_struct: invalid_argument when required strings are empty" {
    vprint("RUNNING: insert_trade_struct: invalid_argument when required strings are empty");
