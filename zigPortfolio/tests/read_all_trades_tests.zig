...
test "read_all_trades: returns all rows in deterministic order" {
    vprint("RUNNING: read_all_trades: returns all rows in deterministic order");
    ...
    // Insert out-of-order to validate ORDER BY in read_all (should be by datetime, id)
    try std.testing.expectEqual(.ok, api.zp_sqlite_insert_trade(handle, "2025-02-01 10:00".ptr, "B".ptr, 1, 1, "IKBR".ptr, "BUY".ptr, 0, "US".ptr, "USD".ptr, 1));
    try std.testing.expectEqual(.ok, api.zp_sqlite_insert_trade(handle, "2025-01-01 10:00".ptr, "A".ptr, 1, 1, "IKBR".ptr, "BUY".ptr, 0, "US".ptr, "USD".ptr, 1));
    try std.testing.expectEqual(.ok, api.zp_sqlite_insert_trade(handle, "2025-03-01 10:00".ptr, "C".ptr, 1, 1, "IKBR".ptr, "BUY".ptr, 0, "US".ptr, "USD".ptr, 1));

    var out: [8]api.zp_trade = [_]api.zp_trade{std.mem.zeroes(api.zp_trade)} ** 8;
    var count: usize = 0;

    const rc = api.zp_sqlite_read_all_trades(handle, out[0..].ptr, out.len, &count);
    try std.testing.expectEqual(.ok, rc);
    try std.testing.expectEqual(@as(usize, 3), count);

    try std.testing.expectEqualStrings("2025-01-01 10:00", zstr(out[0].trade_datetime[0..]));
    try std.testing.expectEqualStrings("A", zstr(out[0].ticker[0..]));

    try std.testing.expectEqualStrings("2025-02-01 10:00", zstr(out[1].trade_datetime[0..]));
    try std.testing.expectEqualStrings("B", zstr(out[1].ticker[0..]));

    try std.testing.expectEqualStrings("2025-03-01 10:00", zstr(out[2].trade_datetime[0..]));
    try std.testing.expectEqualStrings("C", zstr(out[2].ticker[0..]));
}
