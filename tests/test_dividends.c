...
int main(int argc, char **argv) {
    ...
    utax_dividends_row batch[32];
    memset(batch, 0, sizeof(batch));

    for (size_t i = 0; i < N; ++i) {
        batch[i] = UTAX_MOCK_DIVIDENDS[i];
    }

    size_t inserted_n = 0;
    rc = utax_dividends_insert_many(db, batch, N, &inserted_n);
    if (rc != UTAX_OK) {
    ...
    for (size_t i = 0; i < N; ++i) {
        assert(batch[i].dividend_id > 0);
        ids[i] = batch[i].dividend_id;
    }
    ...
    /* batch failure should rollback (violates CHECK: per_share > 0) */
    {
        utax_dividends_row bad[2];
        memset(bad, 0, sizeof(bad));
        bad[0] = UTAX_MOCK_DIVIDENDS[0];
        bad[1] = UTAX_MOCK_DIVIDENDS[1];
        bad[1].per_share = 0.0; /* invalid */
        ...
        size_t done = 0;
        utax_rc rc2 = utax_dividends_insert_many(db, bad, 2, &done);
        assert(rc2 != UTAX_OK);
